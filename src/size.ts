...
function _sizeof( value: any, visited: Set<any> ): number
{
    ...
    switch( typeof value )
    {
        case 'object'   : 
        {
            if( value === null ){ bytes += SIZE.UNDEFINED; break; }
            
            // Check for circular references at the entry of the object block
            ...
            if( Array.isArray( value )){ 
                for( let v of value ){ bytes += _sizeof( v, visited ); } 
            } else
            if( value instanceof Set ){ 
                // Iterate over set values directly to avoid [element, element] wrapper array counting
                for( let v of value.values() ){ bytes += _sizeof( v, visited ); } 
            } else
            if( value instanceof Map ){ 
                for( let [ k, v ] of value.entries() ){ bytes += _sizeof( k, visited ) + _sizeof( v, visited ); } 
            } else
            {
                for( let key in value )
                {
                    ...
                    {
                        bytes += stringSize( key ) + _sizeof( value[ key ], visited );
                    }
                }
            }
            break;
        }
        case 'string'   : bytes += stringSize( value ); break;
