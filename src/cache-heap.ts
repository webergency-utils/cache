...
export default class CacheHeap<T, I = T> extends Heap<T, I>
{
    ...
    public update(item: T, oldItemSize?: number): boolean
    {
        const prevSize = oldItemSize !== undefined ? oldItemSize : this.itemSize( item );
        this.heapSize -= prevSize;
    ...
    public randomTailItem(): T | void
    {
        if( this.data.length )
        {
            ...
            // Corrected: The heap is sorted in ascending order of score (min-heap),
            // so the worst items (with the lowest scores) are at the beginning of the array.
            return this.data[Math.floor( Math.random() * tail_length )];
        }
    }
